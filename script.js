...
// Define categorization field for each layer type
  ...
  fetch(geojsonPaths[type])
    ...
    .then(data => {
      ...
      const getCategoryStyle = (category) => {
        ...
        const radius = 6 + index * 2; // Vary size slightly per category
        const color = categoryColor(index); // Color function
        return {
          radius: radius,
          color: color,
          fillColor: color,
...
// ✅ Improved version to update both map layers and radio button states
document.querySelectorAll('input[name="layer"]').forEach(input => {
  input.addEventListener('change', function () {
    ...
    // ✅ Update radio card active styling
    document.querySelectorAll('.radio-card').forEach(card => {
      card.classList.remove('active');
    });

    // ✅ Set the checked state manually (in case DOM didn't sync)
    this.checked = true;
...
// ✅ Update legend for categorized strings
...
const NorthControl = L.Control.extend({
  options: {
    position: 'topright' // You can change this to 'topleft', 'bottomleft', etc.
  },
  onAdd: function () {
...
function onMapClick(e) {
  ...
  if (distancePoints.length === 2) {
    ...
    // Fetch and show shortest road route using Google API
    getGoogleRoute(latlngs[0], latlngs[1]);
